...
class Auth_Adapter_Facebook implements Zend_Auth_Adapter_Interface
{
    /**
...
//	    	parse_str($result->getBody(), $params);
	    	...
	    	$client = new Zend_Http_Client(Auth_Adapter_Facebook::USER_URI );
	    	$client->setParameterGet('client_id', $this->_appId);
	    	$client->setParameterGet('access_token', $token);
	    	$result = $client->request('GET');
	    	$user = json_decode($result->getBody());
                    
            return new Zend_Auth_Result( Zend_Auth_Result::SUCCESS, $user->id, array('user'=>$user, 'token'=>$token) );
    	}
